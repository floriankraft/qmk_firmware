...
const char *const SIDE_LED_EFF_menu_items[] PROGMEM = {
    SIDE_LED_EFF_TITLE, SIDE_LED_EFF_MENU_1, SIDE_LED_EFF_MENU_2, SIDE_LED_EFF_MENU_3, SIDE_LED_EFF_MENU_4,
};
...
const char *const artwork_menu_items[] PROGMEM = {
    ARTWORK_CFG_TITLE, ARTWORK_CFG_MENU_1, ARTWORK_CFG_MENU_2, ARTWORK_CFG_MENU_3, ARTWORK_CFG_MENU_4,
};
...
static bool go_artwork_setting(void) {
    _load_menu(artwork_menu_items, 1 + eecfg.homeart_id, arrLength(artwork_menu_items));
...
const char *const side_menu_items[] PROGMEM = {SIDE_LED_TITLE, SIDE_LED_MENU_1, SIDE_LED_MENU_2, SIDE_LED_MENU_3, TEXT_AUTO_OFF};
    ...
    if (menuS.list == side_menu_items) {
        switch (menuS.cursor) {
            case 0:
                ...
                break;
            case 1:
                _load_menu(SIDE_LED_EFF_menu_items, 1 + eecfg.side.mode, arrLength(SIDE_LED_EFF_menu_items));
...
static bool go_side_led_setting(void) {
    _load_menu(side_menu_items, 1, arrLength(side_menu_items));
...
const char *const oled_off_menu_items[] PROGMEM = {
    OLED_OFF_TITLE, OLED_OFF_MENU_1, OLED_OFF_MENU_2, OLED_OFF_MENU_3, OLED_OFF_MENU_4, OLED_OFF_MENU_5, OLED_OFF_MENU_6, OLED_OFF_MENU_7,
};

const uint16_t AutoOffCfgValues[] PROGMEM = {0, 10, 60, 300, 600, 1800, 3600};
...
static void go_oled_off_setting(void) {
    _load_menu(oled_off_menu_items, 1 + eecfg.oled.autooff, arrLength(oled_off_menu_items));
...
static void go_side_led_auto_off_setting(void) {
    _load_menu(oled_off_menu_items, 1 + eecfg.side.autooff, arrLength(oled_off_menu_items));
...
static void drawBar(uint16_t apm, uint16_t max) {
    ...
    while (a < (128 / 6)) {
        if ((a % 5) == 0)
            oled_write_char(128, false); // draw grid bar
        else
...
static bool rgb_bright_rot(bool moveDown) {
    if (moveDown)
        eecfg.rgb.bright++;
    else if (!moveDown && eecfg.rgb.bright > 0)
        eecfg.rgb.bright--;
...
static bool rgb_sat_rot(bool moveDown) {
    if (moveDown && eecfg.rgb.satu < 0xff)
        eecfg.rgb.satu++;
    else if (!moveDown && eecfg.rgb.satu)
        eecfg.rgb.satu--;
...
static bool rgb_spd_rot(bool moveDown) {
    if (moveDown && eecfg.rgb.speed < MAX_RGB_SPEED)
        eecfg.rgb.speed++;
    else if (!moveDown && eecfg.rgb.speed)
        eecfg.rgb.speed--;
...
const char *const RGB_EFFECT_menu_items[] PROGMEM = {
    RGB_EFFECT_TITLE, RGB_EFFECT_MENU_1, RGB_EFFECT_MENU_2, RGB_EFFECT_MENU_3, RGB_EFFECT_MENU_4, RGB_EFFECT_MENU_5, RGB_EFFECT_MENU_6, RGB_EFFECT_MENU_7, RGB_EFFECT_MENU_8, RGB_EFFECT_MENU_9, RGB_EFFECT_MENU_10, RGB_EFFECT_MENU_11, RGB_EFFECT_MENU_12, RGB_EFFECT_MENU_13, RGB_EFFECT_MENU_14, RGB_EFFECT_MENU_15,
};
...
const char *const RGB_menu_items[] PROGMEM = {RGBTXT_TITLE, RGBTXT_L1, RGBTXT_L2, RGBTXT_L3, RGBTXT_L4, RGBTXT_L5, RGBTXT_L6, TEXT_AUTO_OFF};
...
static void go_rgb_auto_off_setting(void) {
    _load_menu(oled_off_menu_items, 1 + eecfg.rgb.autooff, arrLength(oled_off_menu_items));
...
static bool rgb_ctrl_keydown(void) {
    switch (menuS.cursor) {
        case 0:
            ...
            break;
        case 1:
            _load_menu(RGB_EFFECT_menu_items, 1 + eecfg.rgb.mode, arrLength(RGB_EFFECT_menu_items));
...
static bool go_rgb_settings(void) {
    app_eeconfig_save();
    _load_menu(RGB_menu_items, 1, arrLength(RGB_menu_items));
...
//         if (star->dis_x > 16 || star->dis_x < -16 || star->dis_y > 10 || star->dis_y < -10) {
...
static void _load_menu(const char *const *list, uint8_t cursor, uint8_t length) {
    ...
    if (length > 8 && cursor > 7) {
        ...
        if (menuS.offset > length - 8) menuS.offset = length - 8;
    }
}
...
static void menu_draw_v_scroll(void) {
    ...
    for (uint8_t c = 1; c <= 2; c++) {
        uint8_t i;
        for (i = 0; i < indicator_pos; i++)
            oled_write_pixel(SCREEN_WIDTH - c, i, false);
        for (; i < indicator_len; i++)
            oled_write_pixel(SCREEN_WIDTH - c, i, true);
...
//     if (eecfg.homeart_id == HOMEART_BONGO) return keydown_bongo_cat(keycode, record);
//     return true;
// }
...
const char *const oled_menu_items[] PROGMEM = {OLED_TITLE, OLED_MENU_1, OLED_MENU_2, TEXT_AUTO_OFF};
...
static bool go_oled_settings(void) {
    _load_menu(oled_menu_items, 1, arrLength(oled_menu_items));
...
const char *const setting_menu_items[] PROGMEM = {
    TXT_TITLE, TXT_MENU_1, TXT_MENU_2, TXT_MENU_3, TXT_MENU_4, TXT_MENU_5, TXT_MENU_6,
};

static bool go_settings(void) {
    app_eeconfig_save();

    _load_menu(setting_menu_items, 0, arrLength(setting_menu_items));
...
bool app_on_key(uint16_t keycode, keyrecord_t *record) {
    ...
    // is normal key press
    if (record->event.pressed) {
        _key_led_i = getLedIdFromKey(record->event.key.row, record->event.key.col);
    }
...
bool app_eeconfig_load(void) {
    // check magicbyte
    if (eeprom_read_byte((uint8_t *)APP_EECONFIG_ADDR_MAGICBYTE) != APP_EECONFIG_MAGICBYTE) return false;
